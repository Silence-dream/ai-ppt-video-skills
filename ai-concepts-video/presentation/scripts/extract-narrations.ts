...
async function main() {
  ...
  for (const { id, folder } of order) {
    ...
    arr.forEach((entry, i) => {
      const step = i + 1;
      // Support both string and {id, text} narration formats
      const text = typeof entry === "string" ? entry : (entry as { id: string; text: string }).text;
      if (typeof text !== "string") {
        throw new Error(
          `chapter "${id}" step ${step}: narration must be a string ` +
            `(got ${typeof text}). The {text, minHoldMs} form was removed; ` +
