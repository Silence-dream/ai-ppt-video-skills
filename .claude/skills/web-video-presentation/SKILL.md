...
description: 把一篇文章或口播稿，做成"看起来像视频"的点击驱动 16:9 网页演示，可选合成口播音频，自动生成提词板用于录制。流程：原始文章 → **一次产出**口播稿 + outline 开发计划 → 用户**一次对齐** 5 件事（稿子 / outline / 主题 / 素材 / 开发模式）→ 网页开发（逐章 / 顺序 / 并行）→ 可选音频合成（默认 MiniMax CLI mmx-cli）→ 提词板生成（BroadcastChannel 双向同步）→ 录屏。**outline 只规划节奏与信息密度，不规划动画** —— 动画由章节开发时按 PRINCIPLES + ANTI-AI 法则即时设计。每次点击推进口播稿的一个节拍，每一步独占整屏，进度条平时隐藏只在悬浮时出现。适用场景：用网页做视频（动态 PPT 但不像 PPT）、把口播稿 / 文章变成可交互的解说、为 B 站 / YouTube / 视频号录屏教程、做有电影感的产品 / talk demo。本 Skill 沉淀的是设计方法论 + 协作流程 —— 不绑定任何特定样式 / 字体 / 颜色 —— 因此能复用到任意主题与美学。
---
...
| Phase 4 提词板生成 | SKILL.md Phase 4 本节（BroadcastChannel 双向同步实现步骤） | —— |
| Phase 5 录屏 + 后期 | `references/RECORDING.md`（含 `?auto=1` 自动录屏） | —— |
...
要不要合成音频做"自动播放录屏"？
  ✓ 合成 → 扫所有章节的 narrations.ts 出 audio-segments.json，
           ...
           本机没装 mmx 会问你用什么 TTS。
  ✗ 不合成 → 跳过 Phase 3，直接 Phase 4 生成提词板。
```

要合成 → Phase 3。不合成 → 直接 Phase 4。

无论是否合成音频，**Phase 4 提词板都是必须步骤**——Phase 3 结束后
自动生成提词板（创建 `src/pages/Ticiban.tsx` + 修改 `App.tsx` 添加
BroadcastChannel 同步 + 配置 Vite SPA 路由），然后进入 Phase 5 录屏。
...
分；太短 = 文案太薄）—— 给最后一次校准节奏的机会。然后进入 Phase 4 生成提词板。
...
> agent 在 Phase 4 提词板生成后**主动告诉用户**录屏路径和提词板地址。
...
| [`references/RECORDING.md`](references/RECORDING.md) | Phase 5 才读 | 录屏工具 + 后期合成 |
...
音频合成（或跳过）后，**必须**生成提词板，供录制时使用。

### 技术方案

使用 **BroadcastChannel API** 实现跨标签页实时通信，无需后端。

```
主演示页面 (http://localhost:PORT/)
    ↕  BroadcastChannel("presentation-sync")
提词板页面 (http://localhost:PORT/ticiban)
```
...
- `step-change`：主演示 → 提词板，广播当前章节和步骤
- `control`：提词板 → 主演示，发送跳转/前进/后退指令
- `ping`：提词板打开时向主页面请求当前状态

### 实现步骤

#### 1. 创建 `src/pages/Ticiban.tsx`

核心设计：

- 显示**全部章节的全部口播文本**，用章节标题分隔
- 当前步骤高亮（蓝色左边框 + 白色文字），其它步骤置灰（opacity 0.3）
- `useRef` 保存 BroadcastChannel 实例（**不能用普通对象，否则每次渲染重建导致消息丢失**）
- `activeRef` + `scrollIntoView({ block: "center" })` 自动滚动到当前条目
- 键盘控制（← → / A D / 空格前进 / R 重置）
- 左右边缘窄条作为点击前进/后退区域
...
useEffect(() => {
  ...
  channel.onmessage = (event) => {
    if (event.data.action === "jump") {
      stepperRef.current.jumpToChapter(targetIdx, targetStep); // ← 通过 ref 调用
    }
  };
}, []);
...
// cursor 变化时广播
useEffect(() => {
  channel.postMessage({ type: "step-change", chapterId, step });
}, [chapterId, step]);
```

#### 3. 配置 Vite SPA 路由

`vite.config.ts` 中设置 `appType: "spa"`，使 `/ticiban` 路径返回 `index.html`。

### 关键陷阱

| 问题 | 原因 | 解法 |
|---|---|---|
| 提词板发消息，主页面不响应 | `useEffect([], [])` 中 `stepper` 被闭包捕获为初始值 | 用 `stepperRef.current` 读最新引用 |
| 提词板消息发不出去 | `channelRef` 是普通对象 `{ current: null }`，每次渲染重建 | 改用 `useRef(null)` |
| 键盘事件处理器读到旧状态 | `useEffect` 依赖数组含 state 导致频繁重注册 | 依赖 `[]`，通过 `currentStepRef` 读最新值 |

### 样式要点

- 章节标题：蓝色（`#3b82f6`）+ 底部分隔线
- 当前步骤：`border-left: 4px solid #3b82f6` + 浅蓝背景
- 非当前步骤：`opacity: 0.3`，hover 时升至 `0.5`
- 左右边缘点击区域：`width: 50px`，`z-index: 5`，叠在滚动内容之上
